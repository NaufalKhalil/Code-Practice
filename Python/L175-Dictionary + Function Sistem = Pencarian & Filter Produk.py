...
# Masukkan kata pencarian: buku
# Masukkan harga maksimum: 20000

# Produk ditemukan:

# -> Buku : Rp.15000
# -> Buku Gambar A4 : Rp.8000
# -> Buku Gambar A3 : Rp.15000

# Jumlah hasil : 3
# Total harga : Rp.38000

def d_input_harga_max():
    while True:
            harga_max = input("Masukkan harga barang : ")
            try:
                harga_max = int(harga_max)
                if harga_max <= 0:
                    print("[Error] Harga barang tidak valid!")
                else:
                    return harga_max
            except ValueError:
                print("[Error] Masukkan sebuah angka/harga barang!")
...
def d_input():
    ...
    harga_max = d_input_harga_max()
    return cari, harga_max

def d_cari(cari, data, harga_max):
    hasil = {}
    for key, value in data.items():
        if cari in key and harga_max >= value:
            hasil[key] = value
    return hasil
...
cari, harga_max = d_input()
data_convert = d_convert_data(data)
hasil = d_cari(cari, data_convert, harga_max)
d_tampilkan(hasil)